...
1. 在 `MachineControlTypes.st` 中新增 `ST_ModuleCommandSource`、`ST_EjectModeCommandSource`、`ST_MachineCommandSourceSet`、`ST_ModuleRuntimeStatus` 及各模块运行时包装结构，把主控内部命令源与运行态边界显式结构化。
...
1. 保持 `FB_PLCDigitalInputBinder` 与 `FB_PLCDigitalOutputBinder` 不变，继续只负责原始 `IN[]/OUT[]` 绑定。
2. 将 `FB_PLCMachineCommandMapper` 的公共运行命令映射逻辑并入 `FB_PLCMachineInputMapper`，把数字量输入语义与公共命令收口到同一个语义层维护。
3. `PRG_MainControl` 与 `Instances.st` 同步改为只通过 `gvl_fbPLCMachineInputMapper` 回写 `gvl_stSensor` 和 `gvl_stPLCDigitalCommandSource`，移除独立 `gvl_fbPLCMachineCommandMapper` 调用链。
