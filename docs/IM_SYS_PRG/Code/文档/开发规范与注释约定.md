...
22. `Code/IO管理` 负责现场原始 IO 绑定；原始数字量点位统一收口到 `Code/IO管理/PLCIOTypes.st` 的 `ST_PLCIO` 与 `GlobalVars.st` 中的 `gvl_PLC`，并由 `PRG_MainControl` 统一先调用数字量输入绑定块、再调用输入语义映射块与公共命令映射块、最后调用数字量输出绑定块。
